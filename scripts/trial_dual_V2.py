...
def calculate_brick_locations():
	...
	robot_offset = [0.6, 0.4, 0.15-0.24, 0]
	bricks_per_layer = 6
	num_layers = 3
	...
	for i in range(num_layers):
		...
		for j in range(int(bricks_per_layer/2)):
			...
			brick_locations_left[brick_number, 3:7] = quaternion_multiply(quaternion_from_euler(0, 0, -math.radians(angles[j + int(bricks_per_layer/2)] + (i % 2) * (360 / (2 * bricks_per_layer)))), overhead_orientation)  # quaternion_from_euler(0,0,-math.radians(angles[j] + i*(360/(2*bricks_per_layer))))
			# quaternion_multiply(quaternion_from_euler(0,0,math.radians(angles[j] + i*(360/(2*bricks_per_layer)))),overhead_orientation)
