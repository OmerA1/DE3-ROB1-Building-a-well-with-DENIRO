...
def calculate_brick_locations_dual():
	...
	robot_offset = [0.69, 0, -0.08, -0.06] #0.15-0.24
	bricks_per_layer = 4
	num_layers = 3
	...
	for i in range(num_layers):
		for j in range(int(bricks_per_layer/2)):
			...
			if i == 1:
				if j == 0:
					new_brick_number = i*int(bricks_per_layer/2)+optimised_route[1]
				if j == 1:
					new_brick_number = i*int(bricks_per_layer/2)+optimised_route[0]
